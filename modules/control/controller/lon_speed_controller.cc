...
Status LonSpeedController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  if (control_conf_ == nullptr) {
    ...
    return Status(ErrorCode::CONTROL_INIT_ERROR,
                  "Failed to load LonSpeedController conf");
  }
  injector_ = injector;
...
Status LonSpeedController::ComputeControlCommand(
    ...
    desired_speed_cmd =
        desired_speed_cmd > 0
            ...
            : std::min(desired_speed_cmd,
                       -control_conf_->low_speed_approach_speed());
    ADEBUG << "Commanding low speed approach ("
           << control_conf_->low_speed_approach_speed()
  ...
  double desired_speed_limited =
      common::math::Clamp(desired_speed_cmd, -speed_controller_input_limit,
                          speed_controller_input_limit);

  // TODO(zero): The speed of the planning module is directional, but the
  // control is converted into gear + unsigned speed
  ...
  // TODO(zero): Temporary settings, reuse variable names
...
void LonSpeedController::ComputeLongitudinalErrors(
