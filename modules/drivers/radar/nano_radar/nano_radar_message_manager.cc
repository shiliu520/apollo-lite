...
void NanoRadarMessageManager::set_radar_conf(RadarConf radar_conf) {
  ...
  if (radar_conf.has_sensor_id()) {
    message_id_offset_ = radar_conf.sensor_id() * 0x10;
  }
}
...
void NanoRadarMessageManager::Parse(const uint32_t message_id,
  ...
  if (!is_configured_ && message_id != RadarState201::ID + message_id_offset_) {
  ...
  if (message_id == RadarState201::ID + message_id_offset_) {
    ADEBUG << sensor_data_.ShortDebugString();
    if (sensor_data_.radar_state().send_quality() ==
      ...
      // Note:
      // It's not recommended to configure sensor_id online, this is
      // meaningless, it will change all connected devices which with the same
      // message ID. The recommended way is to connect and config one by one
      // offline with the offline configure tool
      SenderMessage<NanoRadar> sender_message200(
          RadarConfig200::ID + message_id_offset_, &radar_config_);
      sender_message200.Update();
      can_client_->SendSingleFrame({sender_message200.CanFrame()});
    }
  }

  received_ids_.insert(message_id & 0xFF0F);
